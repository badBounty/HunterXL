...
if [ $WAF -eq 0 ]
	then
		...
		dirsearch -u "$sitio" -w "$(pwd)/content_discovery_all.txt" -o "$(pwd)/dirnfiles.txt" --deep-recursive --force-recursive -e "log,xml,$extensiones" --format=csv -t 60
...
#ZAP
echo "--------------Init zap--------------"
sudo docker run --rm -v $(pwd):/zap/wrk:rw --user root owasp/zap2docker-stable zap-baseline.py -t $sitio -s -j -T 10 -m 5 -a -J zap.json
echo "--------------Init CSV Zap--------------"
